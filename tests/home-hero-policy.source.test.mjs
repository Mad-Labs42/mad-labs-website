...
assert.match(
  index,
  /import\s+MobileHomeHero\s+from\s+["']\.\.\/components\/MobileHomeHero\.astro["'];/,
);
...
assert.ok(
  ...
  "CrtHero guard must run before CRT queries and side effects",
);
...
assert.ok(
  dividerGuardIndex < dividerQueryIndex,
  "CrtDivider guard must run before divider event listeners",
);
...
assert.match(
  glassNav,
  /\.madlabs-glass-nav\.is-crt-taskbar::after[\s\S]*animation:\s*madlabsCrtTaskbarChase\s+11s\s+linear\s+infinite;/,
);
assert.match(
  glassNav,
  /@keyframes\s+madlabsCrtTaskbarChase[\s\S]*transform:\s*translate3d\(-130%,\s*0,\s*0\)[\s\S]*transform:\s*translate3d\(130%,\s*0,\s*0\)/,
);
...
assert.match(
  glassNav,
  /@media\s*\(prefers-reduced-motion:\s*reduce\)[\s\S]*\.madlabs-glass-nav\.is-crt-taskbar::after[\s\S]*animation:\s*none;/,
);

assert.match(
  crtHero,
  /radial-gradient\(\s*ellipse at 50% 42%,\s*rgba\(185, 151, 255, 0\.28\) 0%,\s*rgba\(124, 86, 214, 0\.2\) 34%,\s*transparent 68%\s*\)/,
);
assert.match(
  crtHero,
  /linear-gradient\(\s*180deg,\s*#5c4592 0%,\s*#403073 48%,\s*#2f235c 100%\s*\)/,
);
assert.match(
  crtScreenOverlays,
...
assert.match(
  crtScreenOverlays,
  /:global\(\.is-stable \.crt-screen\) \.crt-phosphor-dots\s*\{[\s\S]*?opacity: 0\.44;[\s\S]*?filter: none;/,
);
